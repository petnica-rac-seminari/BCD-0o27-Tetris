 ...
 * 
 * This code is an adaption of esp_console.h/c by Espressif Systems (Shanghai)
...
// Forward declaration. Definition in ch405_labs_linenoise/ch405_labs_linenoise.h.
typedef struct ch405_labs_linenoiseCompletions ch405_labs_linenoiseCompletions;
...
} ch405_labs_esp_console_config_t;

/**
 * @brief Default console configuration value
 *
 */
#define CH405_LABS_ESP_CONSOLE_CONFIG_DEFAULT() \
    {                                \
...
} ch405_labs_esp_console_repl_config_t;

/**
 * @brief Default console repl configuration value
 *
 */
#define CH405_LABS_ESP_CONSOLE_REPL_CONFIG_DEFAULT() \
{                                         \
        .max_history_len = 32,            \
...
} ch405_labs_esp_console_dev_uart_config_t;

#ifdef CONFIG_ESP_CONSOLE_UART_CUSTOM
#define CH405_LABS_ESP_CONSOLE_DEV_UART_CONFIG_DEFAULT()       \
{                                                   \
...
} ch405_labs_esp_console_dev_usb_cdc_config_t;

#define CH405_LABS_ESP_CONSOLE_DEV_CDC_CONFIG_DEFAULT() \
{                                            \
}
 ...
 */
typedef int (*ch405_labs_esp_console_cmd_func_t)(int argc, char **argv);
...
typedef struct {
     ...
     * The pointer must be valid until the call to ch405_labs_esp_console_deinit.
     */
    const char *command;
    /**
     * Help text for the command, shown by help command.
     * If set, the pointer must be valid until the call to ch405_labs_esp_console_deinit.
     ...
     */
    ch405_labs_esp_console_cmd_func_t func;
    /**
     ...
     * Only used for the duration of ch405_labs_esp_console_cmd_register call.
     */
    void *argtable;
} ch405_labs_esp_console_cmd_t;
 ...
 *      - ESP_ERR_INVALID_STATE, if ch405_labs_esp_console_init wasn't called
 */
...
/**
 * @brief Callback which provides command completion for ch405_labs_linenoise library
 *
 * When using ch405_labs_linenoise for line editing, command completion support
 ...
 * @param lc ch405_labs_linenoiseCompletions to be filled in
 */
void ch405_labs_esp_console_get_completion(const char *buf, ch405_labs_linenoiseCompletions *lc);

/**
 * @brief Callback which provides command hints for ch405_labs_linenoise library
 *
 * When using ch405_labs_linenoise for line editing, hints support can be enabled as
 * follows:
 *
 *   ch405_labs_linenoiseSetHintsCallback((ch405_labs_linenoiseHintsCallback*) &ch405_labs_esp_console_get_hint);
 *
 * The extra cast is needed because ch405_labs_linenoiseHintsCallback is defined as
 ...
 *         not be freed (i.e. ch405_labs_linenoiseSetFreeHintsCallback should not be used).
 */
 ...
 *
 */
typedef struct ch405_labs_esp_console_repl_s ch405_labs_esp_console_repl_t;

/**
 * @brief Console REPL base structure
 *
 */
struct ch405_labs_esp_console_repl_s {
    /**
     ...
     *      - ESP_FAIL on errors
     */
    esp_err_t (*del)(ch405_labs_esp_console_repl_t *repl);
};
 ...
 *       - Initializes ch405_labs_linenoise
 ...
 *            the underlying ch405_labs_linenoise and esp_console functions.
 *
 * @return
 *      - ESP_OK on success
 *      - ESP_FAIL Parameter error
 */
esp_err_t ch405_labs_esp_console_new_repl_uart(const ch405_labs_esp_console_dev_uart_config_t *dev_config, const ch405_labs_esp_console_repl_config_t *repl_config, ch405_labs_esp_console_repl_t **ret_repl);
 ...
 *      - ESP_FAIL Parameter error
 */
esp_err_t ch405_labs_esp_console_new_repl_usb_cdc(const ch405_labs_esp_console_dev_usb_cdc_config_t *dev_config, const ch405_labs_esp_console_repl_config_t *repl_config, ch405_labs_esp_console_repl_t **ret_repl);
 ...
 *      - ESP_FAIL Parameter error
 */
esp_err_t esp_console_new_repl_usb_serial_jtag(const esp_console_dev_usb_serial_jtag_config_t *dev_config, const esp_console_repl_config_t *repl_config, ch405_labs_esp_console_repl_t **ret_repl);
#endif // CONFIG_ESP_CONSOLE_USB_SERIAL_JTAG
 ...
 */
esp_err_t ch405_labs_esp_console_start_repl(ch405_labs_esp_console_repl_t *repl);
