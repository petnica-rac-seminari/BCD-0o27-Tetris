...
#include "esp_log.h"
#define TAG_TFT_SPI_DRIVER              "TFT_SPI_DRIVER"
...
namespace espidf {
    ...
    template<size_t Width, 
            size_t Height,
            size_t PixelDepth, // NEW - TODO VERIFY
    ...
    struct tft_spi_driver {
        ...
        constexpr static const size_t max_transfer_size = MaxTransferSize;
        ...
        bool m_initialized;
        spi_device m_spi;
        spi_device_manager<max_transfer_size, max_transactions, timeout> m_spi_mgr; 
        size_t m_batch_left;
        ...
        tft_spi_driver_result send_next_internal(const uint8_t* data,size_t size,bool queued,bool user,bool skip_batch_commit) {
            ...
            // DEBUG printf("Calling m_spi_mgr.write: size: %d, user: %d, transaction type: %s\n", size, user, (queued)?"spi_transaction_type::queued":((use_polling)?"spi_transaction_type::polling":"spi_transaction_type::interrupt"));
    ...
    public:
        ...
        tft_spi_driver_result frame_write(const tft_spi_driver_rect& bounds,const uint8_t* bmp_data) {
            ...
            if(b.x1>=width || b.y1>=height) {
                // DEBUG printf("Done: b.x1>=width || b.y1>=height");
                return tft_spi_driver_result::success;
            }

            // DEBUG BEGIN
            uint32_t total_transfer_size = (b.x2-b.x1+1) * (b.y2-b.y1+1) * this->pixel_depth; // Coordinates are uint16_6, so max result of multiplication is uint32_t / TODO - esp32 has max of size_t
#ifdef ESP_PLATFORM
            ESP_LOGV(TAG_TFT_SPI_DRIVER, "Normalised values: (%d,%d)-(%d,%d)\n",b.x1,b.y1, b.x2, b.y2); 
            ESP_LOGV(TAG_TFT_SPI_DRIVER, "Total transfer size (max transfer size): %lu (%u)\n", total_transfer_size, max_transfer_size);
#endif // ESP_PLATFORM

            if(total_transfer_size > this->max_transfer_size) { 
                
                // If we try to transfer a frame that is larger than what can be transmitted on the SPI bus in one go,
                // we need to subdivide this frame and send those frame fragments individually. We do this by:
                //      1) max transfer size > width of frame to be transfered: 
                //              Calculate how many transactions of same amount of lines (except last transaction may be 
                //              less) we need to make. Then transfer. 
                //      2) max transfer size < width of frame to be transered:
                //              Segment a line in the minimal amount of equal fragments (except last fragment on the line
                //              which may be smaller) 
#ifdef ESP_PLATFORM
                ESP_LOGV(TAG_TFT_SPI_DRIVER, "Trying to write frame that is larger than max transfer size: %lu (max %u)\n", total_transfer_size, this->max_transfer_size);
#endif // ESP_PLATFORM
                tft_spi_driver_result r = tft_spi_driver_result::success;
                uint16_t line_size = (b.x2 - b.x1 + 1) * this->pixel_depth; 
                uint32_t max_bytes_per_transfer = this->max_transfer_size - 8; // 8 is required for spi padding

                if(this->max_transfer_size > line_size) {
                    // We can transfer at least one full line per frame. Calculate how many fragments we need to 
                    // send.
                    uint16_t max_lines_per_fragment = max_bytes_per_transfer / line_size;
                    uint16_t total_num_lines = (b.y2 - b.y1 + 1);
                    uint16_t num_frame_fragments = total_num_lines / max_lines_per_fragment + (total_num_lines % max_lines_per_fragment ? 1 : 0);
#ifdef ESP_PLATFORM
                    ESP_LOGV(TAG_TFT_SPI_DRIVER, "Max bytes per transfer: %lu\n", max_bytes_per_transfer);
                    ESP_LOGV(TAG_TFT_SPI_DRIVER, "Line size (bytes): %d\n", line_size);
                    ESP_LOGV(TAG_TFT_SPI_DRIVER, "Max lines per fragment: %d\n", max_lines_per_fragment);
                    ESP_LOGV(TAG_TFT_SPI_DRIVER, "Total num lines: %d\n", total_num_lines);
                    ESP_LOGV(TAG_TFT_SPI_DRIVER, "Breaking down in %d frame fragments.\n", num_frame_fragments);
#endif // ESP_PLATFORM
                    uint16_t original_y2 = b.y2;
                    b.y2 = max_lines_per_fragment-1; // 0 height is drawing one line
                    for(uint32_t i = num_frame_fragments; i > 1; i--) {
#ifdef ESP_PLATFORM
                        ESP_LOGV(TAG_TFT_SPI_DRIVER, "Trying to write broken down frame %lu: (%d,%d)-(%d,%d)\n", num_frame_fragments - i, b.x1, b.y1, b.x2, b.y2);
                        ESP_LOGV(TAG_TFT_SPI_DRIVER, "Frame size (max transfer size):       %d (%d)\n", (b.x2-b.x1+1) * (b.y2-b.y1+1) * 2, this->max_transfer_size);
#endif // ESP_PLATFORM
                        r = batch_write_begin_impl(b,false,{1,1,1,1});
                        if(tft_spi_driver_result::success!=r) {
#ifdef ESP_PLATFORM
                            ESP_LOGE(TAG_TFT_SPI_DRIVER, "Could not set coordinates for new frame fragment.");
#endif // ESP_PLATFORM
                            return r;
                        }
                        r = send_next_data(bmp_data,(b.x2-b.x1+1)*(b.y2-b.y1+1)*this->pixel_depth,false,true);
                        if(tft_spi_driver_result::success!=r) {
#ifdef ESP_PLATFORM
                            ESP_LOGE(TAG_TFT_SPI_DRIVER, "Could not write frame fragment %lu.", num_frame_fragments - i);
#endif // ESP_PLATFORM
                            return r;
                        }
                        bmp_data = bmp_data + ((b.x2-b.x1+1) * (b.y2-b.y1+1) * this->pixel_depth);

                        // Adjust y1 for next run
                        b.y1 = b.y2 + 1;
                        b.y2 = b.y1 + max_lines_per_fragment - 1; 
                    }

                    // Now prepare b to point to the remaining data for last fragment
                    b.y2 = original_y2; 

                } else {
                    // We can only transfer parts of a line in one frame, so we need to also divide each line into the
                    // same amount of segments
                    //TODO - not yet implemented
#ifdef ESP_PLATFORM
                    ESP_LOGE(TAG_TFT_SPI_DRIVER, "max spi transfer size smaller than width of frame. Not supported. Disble the copy_from capability on the driver please.");
#endif // ESP_PLATFORM
                    return tft_spi_driver_result::not_supported;
                }
            }
...
#ifdef ESP_PLATFORM
            ESP_LOGV(TAG_TFT_SPI_DRIVER, "Trying to write final frame   : (%d,%d)-(%d,%d)\n", b.x1, b.y1, b.x2, b.y2);
            ESP_LOGV(TAG_TFT_SPI_DRIVER, "Frame size (max transfer size): %d (%d)\n", (b.x2-b.x1+1) * (b.y2-b.y1+1) * 2, this->max_transfer_size);
#endif // ESP_PLATFORM
            ...
            return send_next_data(bmp_data,(b.x2-b.x1+1)*(b.y2-b.y1+1)*this->pixel_depth,false,true);
        }
        ...
        tft_spi_driver_result queued_frame_write(const tft_spi_driver_rect& bounds,
            ...
            // TODO - change code here to adhere to frames larger than the spi max transfer size
